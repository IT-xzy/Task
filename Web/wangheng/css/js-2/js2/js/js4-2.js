...
for (let i = 0; i < arr.length; i++) {
    player += `
        <div class="people">
        <span class="identity"> ${arr[i].breed} </span> 
        <span class="amount"> ${arr[i].num}号 </span> 
        </div>`
}
...
for (let i = 0; i <= arr.length - 1; i++) {
    ...
    if (arr[i].status == "杀死" || arr[i].status == "投死") {
        $('.identity').eq(i).css("background", "#e4e4e4");
        $('.amount').eq(i).css("background", "#e4e4e4");
    }
}

$('button').click(function () {
    location.href = "../html/js4-1.html"
})