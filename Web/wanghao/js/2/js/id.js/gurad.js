...
main.addEventListener("click", function (event) {
    ...
    if (parentX.className == "box-style") { //进行判断父级class名为box-style 则生效
        if (outOject[index].state != "dead") { //判断点击的人是否或活着，活着才能点击
            if (lastEvent == undefined) { //如果是第一次点击的话
                ul.style.display = "flex"; //显示下标
                name.style.background = "red"; //保护颜色
                number.style.background = "red"; //保护颜色
...
footer.addEventListener("click", function () { //设置投票的点击事件
    outOject[index].state = "gurad"; //把对应的对象状态改成被守护
    localStorage.setItem("outOject", JSON.stringify(outOject)); //储存对象数据
    console.log(JSON.parse(localStorage.getItem("outOject")))
    if (night.length < outNumber + 1) {
        let xx = {};
        night.push(xx);
    }
  
    night[outNumber].gurad = index + 1;
    console.log(night);
    localStorage.setItem("night", JSON.stringify(night));
    if(night[outNumber].gurad){
