...
$(".play").on("click", function () { //当点击方块执行以下方法（函数）
    ...
    if ($(this).find(".text").css('background-color') == "rgb(255, 0, 0)") {
...
function kill() {
    ...
    if (
        ...
        arrKill2.push("白天："+arrT[0] + "号被投票杀死，真实身份是"+arrEz[arrT[0]-1]);
        localStorage.setItem('arrKill2', JSON.stringify(arrKill2));
        if (isInArray(arrAll, "杀手alive") && isInArray(arrAll, "平民alive")) {
