...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            '每页显示<input ng-model="conf.itemsPerPage">' +
            '条' +
            '去第<input type="text" ng-model="jumpPageNum" ng-keyup="jumpPageKeyUp($event)"/>' +'页'+
            '</div>' +
            '<div class="no-items" ng-show="conf.totalItems <= 0">暂无数据</div>' +
            '<div class="btn btn-success" ng-click="changeItemsPerPage()">确认</div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
