...
angular.module('admin')
    .controller('ProductCtrC', function ($scope, $state, adminService, $rootScope, $timeout, $filter) {
        var vm = this;
        ...
        var from=vm.from= vm.params.from;
        ...
        vm.sortR = function () {
            $state.go("field.productRecommend", vm.params, {
                reload: true
            });
        };
        ...
        vm.sortSave = function () {
            $rootScope.confirm("确认保存排序？", function () {
                ...
                adminService.productSort(sortPar).then(function (res) {
                    if (res.data.code === 0) {
                        $state.go('field.productCustomer', vm.params, {reload: true});
        ...
        //测试
        //分单产品配置
        vm.updateProConf = function (li) {
            var newData = angular.copy(li);
            if (newData.isConfig == 0) {
                newData.isConfig = 1
            } else {
                newData.isConfig = 0
            }
            ...
            //发送请求
            var str = $filter('proConfigB')(li.isConfig, 'proConfigB');
            $rootScope.confirm('确认' + str + '？', function () {
                adminService.updateProConf(li.id, newData.isConfig).then(function (res) {
                    if (res.data.code == 0) {
                        $rootScope.alert(str + '成功', function () {
                            $state.go($state.current, vm.params, {
                                reload: true
