angular.module('app').controller('professionListCtrl', ['$location', 'professionCount', 'hasHistory', 'dateToNum', '$httpTools', 'ajaxAds', 'getStatusInfo', function ($location, professionCount, hasHistory, dateToNum, $httpTools, ajaxAds, getStatusInfo) {
    var vm = this;
    vm.domData = professionCount;
    vm.history = hasHistory();//历史
    vm.httpSend = vm.history.professionSend || angular.copy(professionCount.defaultHttpData);
    vm.ajaxData = {};
    vm.datepicker = {//日历
        startAtOpen: false,
        endAtOpen: false,
        options: {
            'maxDate': new Date(),
        }
    }
    vm.uibPage = {//分页
        page: 1,
        size: 10,
        totle: 0,
        pageChange: function () {
            vm.httpSend.page = vm.uibPage.page;
            vm.ways.getData();
        },
        toggleFlag: false,
    };
    vm.ways = {
        getData: function () {
            //初始化时间
            vm.httpSend.startAt = dateToNum(vm.httpSend.startAt);
            vm.httpSend.endAt = angular.isNumber(dateToNum(vm.httpSend.endAt)) ? dateToNum(vm.httpSend.endAt) + 84924000 : dateToNum(vm.httpSend.endAt);
            let promise = $httpTools.get(ajaxAds.profession.geiItems, vm.httpSend);
            promise.then(function (Success) {
                if (Success.data.code === 0 && Success.data.message === 'success') {
                    vm.ajaxData = Success.data;//Dom数据
                    //分页优化
                    vm.uibPage.total = vm.ajaxData.total;
                    vm.uibPage.page = vm.httpSend.page;
                    vm.uibPage.size = vm.httpSend.size;
                    if (vm.uibPage.total <= vm.uibPage.size) {
                        vm.uibPage.toggleFlag = true;
                        if (vm.uibPage.total <= 10) {
                            vm.httpSend.size = 10;
                        } else {
                            vm.httpSend.size = vm.uibPage.total;
                        }
                    } else {
                        vm.uibPage.toggleFlag = false;
                    }
                    //地址栏改变url
                    $location.search(function () {
                        var res = {};
                        for (var i in vm.httpSend) {
                            vm.httpSend[i] && (res[i] = vm.httpSend[i]);
                        }
                        return res;
                    }());
                    //本地存储
                    vm.httpSend.endAt = angular.isNumber(dateToNum(vm.httpSend.endAt)) ? dateToNum(vm.httpSend.endAt) - 84924000 : dateToNum(vm.httpSend.endAt);
                    vm.history.professionSend = vm.httpSend;
                    sessionStorage.history = JSON.stringify(vm.history);
                } else {
                    alert(Success.data.message)
                }
            }, function (e) {
                alert('请求错误');
            })
        },
        clearHistory: function () {
            //保存一下size
            let size = vm.httpSend.size;
            angular.copy(angular.copy(professionCount.defaultHttpData), vm.httpSend);
            vm.httpSend.size = size;
            vm.ways.getData();
        },
        deleteItem: function (id) {
            let uibModalpromise = confirm.apply(null, ['是否确认删除?']);
            uibModalpromise.then(function (resolved) {
                //确认之后$http
                let promise = $httpTools.delete(ajaxAds.profession.editItem + id, '');
                promise.then(function (Success) {
                    if (Success.data.code === 0 && Success.data.message === 'success') {
                        alert('删除成功');
                        // 更新dom
                        vm.ways.getData();
                    } else {
                        alert(Success.data.message)
                    }
                }, function (Error) {
                    alert('请稍后重试');
                })
            }, function (rejected) {
                //取消
                alert('操作已经取消')
            })
        },
        changeStatus: function (id, status) {
            let statusInfo = getStatusInfo(function (status) {
                return status === 0 ? true : false
            }(status));
            let send = {
                status: statusInfo.status === true ? 0 : 1,
                id: id,
            }
            let uibModalpromise = confirm.apply(null, statusInfo.content);
            uibModalpromise.then(function (resolved) {
                //确认之后$http
                let promise = $httpTools.put(ajaxAds.profession.itemStatus, send);
                promise.then(function (Success) {
                    if (Success.data.code === 0 && Success.data.message === 'success') {
                        alert('操作成功');
                        // 更新dom
                        vm.ways.getData();
                    } else {
                        alert(Success.data.message)
                    }
                }, function (Error) {
                    alert('请稍后重试');
                })
            }, function (rejected) {
                //取消
                alert('操作已经取消')
            })
        },
    }
    //页面初始化
    vm.ways.getData();
}])