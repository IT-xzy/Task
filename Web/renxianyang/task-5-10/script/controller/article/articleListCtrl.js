angular.module('app').controller('articleListCtrl', ['$location', 'articleCount', '$httpTools', '$state', 'getStatusInfo', 'dateToNum', '$http', 'hasHistory', 'ajaxAds', function ($location, articleCount, $httpTools, $state, getStatusInfo, dateToNum, $http, hasHistory, ajaxAds) {
    var vm = this;
    vm.domData = articleCount;
    vm.ajaxData = {};
    vm.history = hasHistory();//历史
    vm.httpSend = vm.history.articleSend || angular.copy(articleCount.defaultHttpData);
    vm.datepicker = {//日历
        startAtOpen: false,
        endAtOpen: false,
        options: {
