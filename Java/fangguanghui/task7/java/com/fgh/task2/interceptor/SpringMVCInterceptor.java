...
public class SpringMVCInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (token!=null){
            ...
            int userid=TokenUtil.decrypt(tokenValue);
//            String[] s_token=tokenData.split("/");
//            String id=s_token[0];
//            int userid=Integer.parseInt(id);
            //输出用户id
            logger.debug("id:"+userid);
            if (loginService.quaryById(userid)!=null)
                return true;

        }
        //验证失败，返回登录界面
        request.getRequestDispatcher("/WEB-INF/views/smsTest.jsp").forward(request,response);
        logger.debug("登录失败");
        return false;
    }
