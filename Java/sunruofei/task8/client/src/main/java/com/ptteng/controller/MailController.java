    ...
    public String getMessage(String mail) throws IOException {
        ...
        if (RandomUtil.randomCode() == 0) {
            try {
                ...
                mailService = (MailService) applicationContext.getBean("mail");
            } catch (Exception e) {
                ApplicationContext applicationContext = new ClassPathXmlApplicationContext("serverPackage/server1.xml");
                mailService = (MailService) applicationContext.getBean("mail1");
            }
        } else {
            try {
                ApplicationContext applicationContext = new ClassPathXmlApplicationContext("serverPackage/server1.xml");
        ...
        state = mailService.insertCodeMail(mail, message, System.currentTimeMillis());
        ...
        if (!ObjectUtils.isEmpty(name)
            ...
            if (ObjectUtils.isEmpty(userName)) {
                ...
                if (ObjectUtils.isEmpty(user)) {
                    ...
                    state = userService.insertMail(name, password, mail, phone);
