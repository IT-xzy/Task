    ...
    public String getCode(String phone) {
        ...
        if (RandomUtil.randomCode() == 0) {
            try {
                ...
                noteService = (NoteService) applicationContext.getBean("note");
            } catch (Exception e) {
                ApplicationContext applicationContext = new ClassPathXmlApplicationContext("serverPackage/server1.xml");
                noteService = (NoteService) applicationContext.getBean("note1");
            }
        } else {
            try {
                ApplicationContext applicationContext = new ClassPathXmlApplicationContext("serverPackage/server1.xml");
        ...
        //统计小于等于当天结束时间的时间戳
        number = noteService.selectTime(dayStart, dayEnd, Long.valueOf(phone));
    ...
    public String insert(User user, MultipartFile multipartFile, HttpServletResponse response) throws IOException {
        ...
        if (!ObjectUtils.isEmpty(user)
            ...
            if (!ObjectUtils.isEmpty(userName)) {
                ...
                userCodePhone = userService.selectCodePhone(user.getCode(), user.getPhone());
