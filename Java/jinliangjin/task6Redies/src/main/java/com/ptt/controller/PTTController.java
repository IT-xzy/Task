...
//        List<GraduateStudent> goodStudents = homePageService.getGoodStudent(4);
    ...
    public String loginResult(HttpServletResponse response, HttpSession session, Model model, String rememberMe,
        ...
        boolean flag = userService.login(response, user, rememberMe, session);
        if (flag) {
    ...
    public String registerResult(Model model, @Validated(value = IRegisterGroup.class) User user, BindingResult result) throws Exception {
        if (result.hasErrors()) {
            ...
            if (message.equals("fail")) {
                logger.info("输入用户名重复");
                model.addAttribute("message", "输入的用户名已被注册，请重新输入。");
                return "register";
    ...
    public String registerJ(){
        return "registerJson";
    }

    @RequestMapping(value = "/registerJson/json", produces = "text/json;charset=UTF-8")
    @ResponseBody
    public String registerJson(@Validated(value = IRegisterGroup.class) User user, BindingResult result) throws Exception {
        if (result.hasErrors()) {
            logger.info("输入格式错误");
            List<ObjectError> errors = result.getAllErrors();
            for (ObjectError error : errors) {
