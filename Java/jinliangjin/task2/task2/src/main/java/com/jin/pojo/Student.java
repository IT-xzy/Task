 ...
 * @Description:
 * @Author: Jin
 * @CreateDate: 2018/5/9 14:33
 * @UpdateUser:
 * @UpdateDate: 2018/5/9 14:33
 * @UpdateRemark:
 * @Version: 1.0
 */
public class Student {
    //integer整数位数上限，fraction小数位数上限。
    @Digits(integer = 6, fraction = 0, message = "{student.id.error}", groups = ValidateId.class)
    private int id;
    @Size(min = 1, max = 8, message = "{student.name.length.error}", groups = ValidateAll.class)
    private String name;
    private String sex;
    private Long qq;
    ...
    @URL(message = "{student.url.error}", groups = ValidateAll.class)
    private String link;
