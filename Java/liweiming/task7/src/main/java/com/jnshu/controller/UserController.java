...
import com.jnshu.service.impl.BusinessServiceImpl;
...
//        MD5加盐加密密码
    ...
    public Object sendEmail(String username, String email, HttpServletRequest request, HttpServletResponse response) throws com.aliyuncs.exceptions.ClientException {
        JSONObject json = new JSONObject();
        ...
        String token = null;
        try {
            token = JWTUtil.emailToken(username + ":" + email);
        } catch (Exception e) {
            log.error("密钥生成失败！", e);
        }
    ...
    public String viePhoto(HttpServletRequest request, Model model){
        ...
        UserDTO student = businessService.getByUsername(username);
        if (student == null){
            student = businessService.getByEmail(username);
            if (student == null){
                student = businessService.getByPhone(username);
                if (student == null){
                    log.error("该学员不存在");
                    return "error";
                }
