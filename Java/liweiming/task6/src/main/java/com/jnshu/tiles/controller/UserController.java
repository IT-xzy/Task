...
import com.jnshu.tiles.service.impl.BusinessServiceImpl;
...
//        MD5加盐加密密码
    ...
    public ModelAndView login(HttpServletRequest request, HttpServletResponse response,
        ...
        if (businessService.findByUserName(username)==null){
            ...
            //  获取用户名，在数据库中
            UserDTO userDTO = businessService.login(username);
            ...
            if (Md5Util.getSaltverifyMD5(password,psw)){
               // 登陆成功，将user对象设置到HttpSession作用范围域中
                ...
                try {
                    String tokens = JWTUtil.createToken(userDTO);
                ...
                /*//ID和登录时间作为原密文
                String str = userDTO.getId() + ":" + System.currentTimeMillis();
                log.info("原密文：" + str);
                //进行DES加密
