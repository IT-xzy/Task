...
public class SmsController {
  ...
  @RequestMapping(value = "/a/getPhone", method = RequestMethod.POST)
  public String getCode(String phone) {
    ...
    if (RandomUtil.randomCode() == 0) {
      try {
        ...
        mobileService = (MobileService) applicationContext.getBean("mobile");
      } catch (Exception e) {
        ApplicationContext applicationContext =
            new ClassPathXmlApplicationContext("serverPackage/server1.xml");
        mobileService = (MobileService) applicationContext.getBean("mobile1");
      }
    } else {
      try {
        ApplicationContext applicationContext =
  ...
  //    // 统计小于等于当天结束时间的时间戳
  //    number = mobileService.selectTime(dayStart, dayEnd, Long.valueOf(phone));
  ...
  //      log.info("result====" + result);
  //
  //      String sendResult = smsUtil.setSms(String.valueOf(code), String.valueOf(phone));
  //      log.info("sendResult====" + sendResult);
  //      return "register";
  //    }
  ...
  public String insert(User user, MultipartFile multipartFile, HttpServletResponse response)
    ...
    if (!ObjectUtils.isEmpty(user)
      ...
      userName = userService.selectByName(user.getName());
