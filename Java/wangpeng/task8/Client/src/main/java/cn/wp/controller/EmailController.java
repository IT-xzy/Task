...
public class EmailController {
  ...
  @RequestMapping(value = "/a/gatMail", method = RequestMethod.POST)
  public String getMessage(String mail) throws IOException {
    ...
    if (RandomUtil.randomCode() == 0) {
      try {
        ...
        emailService = (EmailService) applicationContext.getBean("email");
      } catch (Exception e) {
        ApplicationContext applicationContext =
            new ClassPathXmlApplicationContext("serverPackage/server1.xml");
        emailService = (EmailService) applicationContext.getBean("email1");
      }
    } else {
      try {
        ApplicationContext applicationContext =
    ...
    state = emailService.insertCodeMail(mail, message, System.currentTimeMillis());
   ...
   */
  @RequestMapping(value = "/a/registerByMail", method = RequestMethod.POST)
  public String register(HttpServletRequest request) {
    ...
    if (!ObjectUtils.isEmpty(name)
      ...
      // 用户名是否存在
      userName = userService.selectByName(name);
      logger.info("用户名是========" + userName);
      if (ObjectUtils.isEmpty(userName)) {
        // 用户名不存在,可注册
        // 查找email表中是否有发送的邮件和验证码
        user = userService.selectCodeMail(code, mail);
        ...
        if (!ObjectUtils.isEmpty(user)) {
          ...
          state = userService.insertMail(name, password, phone, mail);
