...
public class Register {
...

    {
        ...
        if(traineesFine==null){
            ...
            trainees.setPassword(MD5Util.MD5(password+traineesFine.getSalt()));
        ...
        Trainees trainees=new Trainees();
        trainees.setPassword(password);
        trainees.setAccount(account);
        int i =serviceIF.enterPage(trainees);
        //得到执行的情况；0 ——能登录，1 ——密码错误，2 ——没有这个账号；其它，异常情况
        switch (i){
            case 0:
                //用Tocken对用户ID进行加密
                Trainees tr=serviceIF.checkPwd(account);
                Cookie cookie=new Cookie("name",Tocken.tocken(tr.getId()));
                response.addCookie(cookie);
                return "fpass";
            case 1:
                model.addAttribute("a","密码错误，请重新登录");
