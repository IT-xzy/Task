...
import com.util.task7.EmailUtil;
...
//            userService.updatePeople(people);
    ...
    public String register(People people, HttpServletRequest request, Result result) {
        ...
        //MD5加密加盐，加了固定的字符串
        people.setPassword(DigestUtils.md5Hex(people.getPassword()) + "abc");
        if (noteUtil.checkRandCode(randCode, tel)) {
            logger.info("短信验证成功");
            people.setStatus(1);
    ...
    public String sendCode(Long tel, Result result) {
        ...
        //获取随机验证码
        String rand_Code = NoteUtil.getRangCode();
        //查询code表，手机号信息验证
        Code code = noteUtil.checkTelSum(tel);
        if (code == null) {
            //次数超过5次
            result.setCode(-101);
        ...
        if (noteUtil.sendRandCode(code, rand_Code)) {
            result.setCode(0);
            result.setMsg("发送成功");
        } else {
            result.setCode(-100);
    ...
    public String emailRegister(People people, HttpServletRequest request, Result result) {
        ...
        //发送邮件
        if (emailUtil.sentEmail(request, randCode, emailType)) {
            //MD5加密加盐，加了固定的字符串
            people.setPassword(DigestUtils.md5Hex(people.getPassword()) + "abc");
            people.setCreatTime(System.currentTimeMillis());
