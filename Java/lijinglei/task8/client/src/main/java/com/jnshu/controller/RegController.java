...
//   static UserService rmiClient.getUserService();
//
//    public void setUserService(UserService rmiClient.getUserService()) {
//        RegController.rmiClient.getUserService() = rmiClient.getUserService();
//    }
    RMIClient rmiClient =RMIClient.server();
//    UserService rmiClient.getUserService() = rmiClient.getUserService();
    @Autowired
    COSClient cosClient;
    ...
    public void email(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        if (codeRecode != null && codeRecode != "f") {
            logger.info("\n缓存" + codeRecode);
        ...
        if (dateDifference >= 5 * 60 * 1000 && times <= 5) {
            ...
            try {
                cache.setCacheObject(addr+"code", code, Long.valueOf(validity));
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (addr.indexOf("@") != -1) {
    ...
    public void checkCode(HttpServletRequest request, HttpServletResponse response)
        ...
        String redisRs = cache.getCacheObject(telNum+"code") + "," + cache.getCacheObject(email+"code");
    ...
    public void checkUser(HttpServletRequest request, HttpServletResponse response)
        ...
        User user = rmiClient.getUserService().selectByName(name);
        if (user == null) {
            out.print(1);
        } else {
    ...
    public String add(User user, HttpServletRequest request) {
        try {
            ...
            String str = rmiClient.getUserService().addInfo(user);
            logger.info(str);
            ...
            if (type != null) {// 判断文件类型是否为空
                if ("GIF".equals(type.toUpperCase()) || "PNG".equals(type.toUpperCase()) || "JPG".equals(type.toUpperCase())) {
                    ...
                    if (se != null) {
                        ...
                        //文件名上传到数据库
                        String re = rmiClient.getUserService().uploadImage(username, trueFileName);
