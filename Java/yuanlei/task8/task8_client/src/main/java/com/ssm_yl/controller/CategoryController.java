...
//    CategoryService categoryService;
    ...
    private static CategoryService selectServer(){
         if(Math.random()>0.5&&flagA==0){
            try {
                categoryService = (CategoryService) context.getBean("clientRmi1");
                categoryService.list();
                System.out.println("正在使用server1");
            }catch (Exception e){
                flagA = 1;
                categoryService =(CategoryService)context.getBean("clientRmi2");
    ...
    public ModelAndView list( @RequestParam(required=true,defaultValue="1") Integer pageNum) {
