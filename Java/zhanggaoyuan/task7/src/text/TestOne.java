...
@ContextConfiguration(locations={"classpath:springConfig/spring-mybatis.xml", "classpath:springConfig/spring-redis.xml"})
...
RestUser list=studentService.selectUserByName ("小红a");
logger.info (JSON.toJSONString (list));
    }


    @Test
    public void select333() {
        RestUser user = new RestUser ();
        user.setName ("张小愿");
        user.setEmail ("123@456");
        int list=studentService.updateByName (user);
        logger.info (JSON.toJSONString (list));
    }
...
//        user.setPhone (12355);
        ...
        int rs=studentService.insertUser (user);
        logger.info (rs);
    }
//
//    @Test
