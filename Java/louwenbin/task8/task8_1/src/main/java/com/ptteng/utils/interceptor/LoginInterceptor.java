package com.ptteng.utils.interceptor;
...
public class LoginInterceptor implements HandlerInterceptor {
    //@Autowired
    //private UserServiceLogin userServiceImplLogin;
    @Resource
        ...
        if (null != cookies) {
            for (Cookie cookie : cookies) {
                if ("name".equals(cookie.getName())) {
                    ...
                    logger.info("the token is {}", loginCookieUserName);
                    username = userParams[0];
                    time = userParams[1];
                    long oldTime = Long.parseLong(time);
                    long nowTime = System.currentTimeMillis();
                    UserLogin userDB = userServiceImplLogin.selectUserLogin(username);
                    if (nowTime - oldTime < 30 * 60 * 1000 || nowTime - oldTime > 0) {
                        if (username.equals(userDB.getAccount())) {
                            return true;
                        }
                    }
                }
