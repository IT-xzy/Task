...
public class UserControllerLogin {
    ...
    //UserServiceLogin userServiceImplLogin = (UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3");
    ...
    public String insertUser(UserLogin user, Model model) {
        ...
        UserLogin userDB = ((UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3")).selectUserLogin(account);
        ...
        if (userDB != null) {
            ...
            user.setSalt(salt);
            ((UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3")).insertUserLogin(user);
    ...
    public String insertUser(HttpServletRequest request, UserLogin user, Model model) {
        ...
        if (codeSession.trim().equalsIgnoreCase(code)) {
            if (userDB != null) {
                ...
                user.setPassword(password);
                user.setSalt(salt);
                ((UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3")).insertUserLoginPhoneCode(user);
                model.addAttribute("返回通知", "注册成功,请登陆");
    ...
    public String email(UserLogin user, Model model, HttpServletResponse response, HttpServletRequest request) throws IOException {
        ...
        if (codeSession.trim().equalsIgnoreCase(code)) {
            if (userDB != null) {
                ...
                user.setPassword(password);
                user.setSalt(salt);
                ((UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3")).insertUserLoginEmailCode(user);
                model.addAttribute("返回通知", "注册成功,请登陆");
    ...
    public String upDemo(UserLogin user, HttpServletRequest request, @RequestParam("file") CommonsMultipartFile commonsMultipartFile) throws Exception {
        ...
        user.setPhoto(name);
        //存储到数据库
        ((UserServiceLogin) myRMIClient.object( "myRMIServer1-3", "myRMIServer2-3")).updateUserLoginImage(user);
