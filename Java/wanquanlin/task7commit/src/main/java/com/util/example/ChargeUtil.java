...
public class ChargeUtil {
    ...
    public Charge createCharge() {
        ...
        chargeMap.put("amount", 100);//订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填 100）REQUIRED
        chargeMap.put("currency", "cny");//商品描述信息，该参数最长为 128 个 Unicode 字符。 REQUIRED
        chargeMap.put("subject", "Your Subject");//商品标题，该参数最长为 32 个 Unicode 字符。REQUIRED
        chargeMap.put("body", "Your Body");//商品描述信息，该参数最长为 128 个 Unicode 字符。 REQUIRED
        String orderNo = new Date().getTime() + Main.randomString(7);
        chargeMap.put("order_no", orderNo);//REQUIRED商户订单号，适配每个渠道对此参数的要求，必须在商户的系统内唯一。 推荐使用 8-20 位，要求数字或字母，不允许其他字符
        chargeMap.put("channel", channel);// 支付使用的第三方支付渠道取值，请参考：https://www.pingxx.com/api#api-c-new
        chargeMap.put("client_ip", "127.0.0.1"); // 发起支付请求客户端的 IP 地址，格式为 IPV4，如: 127.0.0.1。REQUIRED
        Map<String, String> app = new HashMap<String, String>();
        app.put("id", appId);//支付使用的  app 对象的  id EXPANDABLE,REQUIRED
