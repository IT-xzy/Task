...
public class LoginController
{
    ...
    //注册确认
    @RequestMapping(value = "/loginRegisterVerify",method =RequestMethod.PUT)
    public ModelAndView loginRegisterVerify(Login login)
    {
        if(login.getLoginId()!=null)
        {
            ...
            // id不为空，查询密码是否存在，存在则账号存在，注册失败
                ...
                ModelAndView modelAndView=new ModelAndView("redirect:/loginRegisterFailed");
                return modelAndView;
            }
            else
            {//判断一个字符串里含不含有空格，包括未输入任何字符的情况；
                ...
                else
                {
                    ...
                    ModelAndView modelAndView=new ModelAndView("redirect:/loginRegisterSuccess");
                    return modelAndView;
                }
            }
        }
    ...
    // 密码修改确认类
    @RequestMapping(value = "/loginUpdateVerify",method = RequestMethod.POST)
    ...
    public ModelAndView loginUpdate()
    {
    ...
    /*将注册失败的put请求，用这两个controller类转换为普通请求返回到jsp页面；
    jsp页面不能接受put请求
    这两个类作为注册的中转，所以没有参数接受*/
    @RequestMapping("/loginRegisterFailed")
    public ModelAndView loginRegisterFailed()
    {
        ModelAndView modelAndView=new ModelAndView("loginRegisterFailed");
        return modelAndView;
    }
    @RequestMapping("/loginRegisterSuccess")
    public ModelAndView loginRegisterSuccess()
    {
