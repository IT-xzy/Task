...
public class UserServiceTest {
    ...
    public void testR()throws Exception{
       long exp;//过期时间，单位s
        long curTime=System.currentTimeMillis();//当前时间戳，单位ms
        long curDay=curTime/(24*60*60*1000);//当前天数，距离1970年1月1日0:0:0的天数
        long expDay=curDay+1;//过期天数，当前天数加1
        exp=(expDay*24*60*60*1000-curTime)/1000;//过期时间就是过期天数转化为时间戳减去当前时间戳的差，再转化为秒

    }
    @Test
    public void testEmail ()throws Exception{
        EmailUtil.singleSent("463282453@qq.com","123456");
        System.out.println("发送成功");
    }
    @Test
    public void testRest(){
        String url="email=463282453@qq.com&code=123456";
        Map<String,String> map= CRequest.URLRequest(url);
        System.out.println(map.get("email"));
        System.out.println(map.get("code"));
      //SQL注入举例
        //输入管理员账户和密码之后select * from users where username='tarena' and password=md5('admin')
      //如果存在SQL注入漏洞的话，可以通过构造特殊字符串
