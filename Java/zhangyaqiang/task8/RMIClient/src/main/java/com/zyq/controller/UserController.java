...
public class UserController {
    ...
    private RmiUrlListProxyFactoryBean rmiProxyFactoryUserBean;
    private UserService userService;

    @Autowired
    @Qualifier("excellentStudentService")
    private RmiUrlListProxyFactoryBean rmiProxyFactoryExcellentStudentBean;
    private ExcellentStudentService excellentStudentService;

    @RequestMapping(value = "/user",method = RequestMethod.POST)
    public String Resister(User user, Model model){
        userService = (UserService) rmiProxyFactoryUserBean.getObject();
        logger.info("这是注册用户。。。。。。。。。。。");
    ...
    public String Login(User user, Model model, HttpSession session, HttpServletResponse response) {
        userService = (UserService) rmiProxyFactoryUserBean.getObject();
        excellentStudentService = (ExcellentStudentService) rmiProxyFactoryExcellentStudentBean.getObject();
        logger.info("这是用户登录。。。。。。。。。。。。。。。");
        User user1 = null;
