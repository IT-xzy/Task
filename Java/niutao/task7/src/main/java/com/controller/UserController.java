...
import com.service.*;
import com.util.MD5;
import com.util.RandomNumAndChar;
...
public class UserController {
    ...
    public String hello(Model model, HttpServletRequest httpServletRequest,HttpServletResponse httpServletResponse) throws ServletException, IOException {
        boolean islogin = TokenVarify.doVarify(httpServletRequest);
...
//    }
    ...
    public String ulogout(HttpServletRequest httpServletRequest,HttpServletResponse httpServletResponse){
        Cookie cookie = GetcookieByName.dogetcookie(httpServletRequest,"logtime");
        cookie.setPath("/");
        cookie.setMaxAge(0);
    ...
    { int id = FromCookieGetId.getId(request);
        ...
        model.addAttribute("url",selectOOS.geturl()+student.getHeadPicture());
        return "userhome";
    }
    //用户信息更改。
    ...
    public String uhomepost( HttpServletRequest request,RedirectAttributes redirectAttributes, @Validated({Update.class}) Student student, BindingResult bindingResult){
        if(bindingResult.hasErrors()){
            ...
            return "redirect:/u/home";          //"redirect:/u/home"; ？？？？？？？？还没搞清楚
        }
    ...
    public ModelAndView verifycode(String facecode,HttpServletRequest httpServletRequest) {
        String phoneverify = GetcookieByName.dogetvalue(httpServletRequest,"phoneverify");
        ModelAndView mav = new ModelAndView(new MappingJackson2JsonView());
        //用户填写数据与发送数据对比
        if (facecode == null || facecode == "") {
