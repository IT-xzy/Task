...
public class SecondWorkController {
    ...
    public ResultBean insertBanner(SecondWork record) {
        logger.info("----添加被调用-----");
        logger.info("：添加的数据" + record);
        ResultBean resultBean = new ResultBean();
        int x = secondWorkService.insert(record);
        logger.info("结果" + x);
        if (x == 0) {
            resultBean.setCode(-1);
    ...
    @ResponseBody
    @RequestMapping(value = "/second/{id}", method = RequestMethod.PUT)
    public ResultBean updateBanner(@RequestParam Long id, SecondWork record) {
        logger.info("----更新方法被调用-----");
        logger.info("：更新内容" + record);
        ResultBean resultBean = new ResultBean();
        int x = secondWorkService.updateByPrimaryKey(record);
        if (x == 0) {
            resultBean.setCode(-1);
    ...
    public ResultBean deleteBanner(@PathVariable Long id) {
        ...
        ResultBean resultBean = new ResultBean();
        int x = secondWorkService.deleteByPrimaryKey(id);
        if (x == 0) {
            resultBean.setCode(-1);
