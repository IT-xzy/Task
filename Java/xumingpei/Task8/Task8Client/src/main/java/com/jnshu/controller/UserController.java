...
import com.jnshu.tool.AliyunOss;
import com.jnshu.tool.Cos;
...
public class UserController {
    ...
    public String insert(User user, HttpServletResponse response, String msgCode) throws IOException {
        ...
        if (!ObjectUtils.isEmpty(user.getName()) && !ObjectUtils.isEmpty(user.getPassword())) {
            ...
            //比对验证码是否一致 0为一致
            else if (rmiClient.getUserService().checkPhoneCode(phone, msgCode) == 0){
                ...
                if (ObjectUtils.isEmpty(userName)){
                    ...
                    user.setPhone(phone);
                    int regist = rmiClient.getUserService().insert(user);
    ...
    public String registByEmail(User user, HttpServletResponse response, String msgCode) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(user.getName()) && !ObjectUtils.isEmpty(user.getPassword())) {
            if (msgCode == null) {
                ...
                return "register";
            } else if (rmiClient.getUserService().cheakEmail(email, msgCode) == 0) {
                //注册时,先根据用户名查,如果查不出,说明数据库里没有这条数据则插入,否则返回注册页面
                List<User> userName = rmiClient.getUserService().selectByName(user.getName());
                logger.info("用户名是："+userName);
                if (ObjectUtils.isEmpty(userName)){
    ...
    public ModelAndView login(User user , HttpServletResponse response, String name) throws UnsupportedEncodingException {
        ...
        if (name.contains("@")) {
            ...
            user = rmiClient.getUserService().selectByEmail(name);
            ...
            user = rmiClient.getUserService().selectByPhone(name);
    ...
    public String phoneCode (String phone){
        ...
        int status = rmiClient.getUserService().sendPhone(phone, msgCode);
    ...
    public String emailCode(String email) throws IOException {
        ...
        logger.info("验证码："+msgCode);
        int status = rmiClient.getUserService().sendEmail(email,msgCode);
