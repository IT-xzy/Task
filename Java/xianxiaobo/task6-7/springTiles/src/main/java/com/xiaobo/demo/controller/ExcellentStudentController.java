...
public class ExcellentStudentController {
    ...
    public Object postRegister(@RequestBody JSONObject param){
        ...
        String link = emailCodeService.encryptActiveUrl(user.getId());
    ...
    public Object getCaptcha(@RequestParam(value="vaptcha_token") String vaptchaToken,
        ...
        if(phoneCode!=null){
            ...
            // 判断今日发送条数
            if(commonService.isToday(phoneCode.getUpdateAt())&&phoneCode.getNumber()>9){
            ...
            newPhoneCode.setNumber(commonService.isToday(phoneCode.getUpdateAt())?phoneCode.getNumber()+1:0);
            ...
            if(!phoneCodeService.updateByPrimaryKeySelective(newPhoneCode)){
        ...
        if(phoneCode==null){
            ...
            newPhoneCode.setUpdateAt(System.currentTimeMillis());
            if(phoneCodeService.insert(newPhoneCode)<=0){
    ...
    public ModelAndView getEmailActive(@RequestParam(value="param") String param){
        if(emailCodeService.decryptActiveUrl(param)){
...
//                    String newFileName = new Date().getTime() + String.valueOf(userId)+file.getOriginalFilename();
    ...
    public Object getMyInfo(HttpServletRequest request){
        ...
        User user = new User();
        user.setId(userId);
        User result = userService.getUserWithoutPassword(user);
