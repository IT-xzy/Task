...
public class UserController {
    ...
    public String getmessage(String phone) {
        ...
        if (!ObjectUtils.isEmpty(phone)) {
            ...
            rolianUtil.rolian(phone, code);
            logger.info("input ronglian");
            logger.info("phone=====" + phone + "code====" + code);
            Boolean a = userService.insertByPhone(phone, code);
            logger.info("input db code" + a);
            return "register";

        } else {
            logger.info("not phone ");
            return "/a/message";
        }
    }
...
//进行判断，输入的信息有User集合的全部信息，和单独用来判断的那么信息，一开始想的是先将User集合中的name用get方法取出来，进行判断，现在是只需要直接将User，name传输进来
    ...
    public String insert(User user) {
        ...
        if (!ObjectUtils.isEmpty(user.getName()) && !ObjectUtils.isEmpty(user.getPassword())
                && !ObjectUtils.isEmpty(user.getPhone()) && !ObjectUtils.isEmpty(user.getCode())
        ) {
            logger.info("输入的账户信息===============" + user);
//        用name来查询数据库中是否有该用户存在的信息，
            List<User> users = userService.selectByName(user.getName());
...
//        进行判断user表中有没有这个用户，有的话直接跳转注册界面，没有的话进行注册
            if (!CollectionUtils.isEmpty(users)) {
                ...
                List<User> userss = userService.selectByPhone(user.getPhone(), user.getCode());
                if (!ObjectUtils.isEmpty(userss)) {
