...
public class StudentController {
    ...
    public String deleteStudent(@PathVariable Long id,Model model,Page page) {
        ...
        int total = studentService.total();
        page.calculateLast(total);
        logger.info("删除数据的页码："+page.getStart());

        deleteTag = studentService.deleteStudent(id,page);
    ...
    public ModelAndView updateStudent(Student student,Page page) {
        ModelAndView mav  = new ModelAndView();
        int total = studentService.total();
        page.calculateLast(total);
        if(student.getId()==null){
            logger.info("id不存在，添加信息");
            logger.info("新增数据的页码："+page.getStart());

            studentService.insertStudent(student,page);
        }else {
            logger.info("id已经存在，更新信息");
            logger.info("更新数据的页码："+page.getStart());
            boolean tag = studentService.updateStudent(student,page);
            if(tag){
                logger.info("更新成功");
            }else{
                logger.info("更新失败");
