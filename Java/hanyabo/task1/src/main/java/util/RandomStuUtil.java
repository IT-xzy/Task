...
public class RandomStuUtil {

	// public static void main(String[] args){
	// System.out.println(getRandomStu());
	// }

	private static Student stu = new Student();
	private static Random random = new Random(System.currentTimeMillis());

	public static Student getRandomStu() {
		stu.setName(RName());
		stu.setProfession(RCourse());
		stu.setQq("1546464646");
		stu.setJoin_date(new Date());
		stu.setSchool("克莱登大学");
		stu.setOnline_id("散修弟子" + stu.getProfession() + random.nextInt(9999));
		stu.setDaily_url("http://www.jnshu.com/school/11455/daily");
		stu.setDeclaration("老大最帅");
		stu.setIntroducer(RName());
		stu.setReferee(RName());
		stu.setCounselor(RName());
		stu.setDescription("");
		stu.setCity(RCity());
		stu.setReview("待审核");
		return stu;
	...
	public static void main(String[] args) {
		System.out.println(RandomStuUtil.getRandomStu().toString());
	}
}
