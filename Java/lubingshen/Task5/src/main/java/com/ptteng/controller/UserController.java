...
import com.ptteng.utils.DateTrans;
...
public class UserController {
    ...
    public String login(HttpServletRequest request, HttpServletResponse response, User user) throws Exception {
        ...
        logger.info("登录信息： 用户名：" + user.getUserName() + "  用户密码：" + user.getUserKey()
                + "  登录时间：" + DateTrans.longToString(current, "yyyy年MM月dd日 HH时mm分ss秒"));
    ...
    public String tryToRegister(User user) throws Exception {
        ...
        logger.info("注册信息： 用户名：" + user.getUserName() + "  用户密码：" + password
                + "  注册时间：" + DateTrans.longToString(System.currentTimeMillis(), "yyyy年MM月dd日 HH时mm分ss秒"));
