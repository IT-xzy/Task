...
加了这个就不需要显式实现接口了*/
@Controller
@RequestMapping(value = "/a/u/task2")
public class StudentController {
    @Autowired
    private StudentService studentService;
    private static final Logger logger = LoggerFactory.getLogger(StudentController.class);

/*  @ControllerAdvice标签的使用方法（异常处理,标签的方式对代码的侵入性太强，不推荐使用） ：
    ...
    @ModelAttribute
    public void addAttributes(Model pojo) {
        pojo.addAttribute("author", "Magical Sam");
    }
     全局异常捕捉处理
    @ResponseBody
    ...
    public String findAStudentByName(@RequestParam String name, Model model) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("姓名查询接收参数:" + name);
        }
        if (name==null||name.equals(""))
            throw new StudentException("请输入有效搜索信息");
        if(!DataCheckUtil.isNotIllegal(name))
            throw new StudentException("搜索内容（姓名）含有非法内容");
        long start = System.currentTimeMillis();
    ...
    public String tryToAdd(Student student) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("新增学员接收参数:" + student);
        }
        long start = System.currentTimeMillis();
        DataCheckUtil.check(student, DataCheckUtil.INSERT);
        long cost = System.currentTimeMillis() - start;
        if (logger.isInfoEnabled()) {
    ...
    public String tryToUpdate(Student student) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("更新学员接收参数:" + student);
        }
        long start = System.currentTimeMillis();
        DataCheckUtil.check(student, DataCheckUtil.UPDATE);
